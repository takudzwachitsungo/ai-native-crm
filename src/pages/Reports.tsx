      ...
      <PageLayout>
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <button onClick={exportCurrentReport} className="inline-flex h-8 w-fit items-center gap-1.5 rounded-full border border-border/70 bg-background px-3 text-xs font-medium text-foreground transition-colors hover:border-primary/30 hover:bg-secondary/60"><Icons.Download size={14} />Export CSV</button>
          </div>
        </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          ...
          <div className="flex flex-col gap-3 lg:flex-row">
            ...
            <button onClick={() => generateReport('custom', selectedMode, customQuery)} disabled={!customQuery.trim() || isGenerating} className="inline-flex h-10 items-center justify-center gap-1.5 rounded-full bg-primary px-4 text-xs font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{isGenerating ? <Icons.RefreshCw size={14} className="animate-spin" /> : <Icons.Sparkles size={14} />}Generate</button>
            <button onClick={handleSaveDefinition} className="inline-flex h-10 items-center justify-center rounded-full border border-border/70 bg-background px-4 text-xs font-medium text-foreground transition-colors hover:border-primary/30 hover:bg-secondary/60">Save Definition</button>
          </div>
