...
public class AccountController {
    ...
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "Get web push configuration for the current account")
    public ResponseEntity<WebPushConfigResponseDTO> getPushConfig() {
        return ResponseEntity.ok(accountSelfService.getPushConfig());
    }

    @PutMapping("/push/subscription")
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "Register or refresh the current browser push subscription")
    public ResponseEntity<WebPushSubscriptionResponseDTO> registerPushSubscription(
            @Valid @RequestBody WebPushSubscriptionRequestDTO request
    ) {
        return ResponseEntity.ok(accountSelfService.registerPushSubscription(request));
    }

    @DeleteMapping("/push/subscription")
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "Remove the current browser push subscription")
    public ResponseEntity<Void> removePushSubscription(@RequestParam String endpoint) {
        accountSelfService.removePushSubscription(endpoint);
